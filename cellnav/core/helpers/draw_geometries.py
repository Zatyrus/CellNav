...
def draw_geometries(
    geometries: List[
        Union[o3d.geometry.TriangleMesh, o3d.geometry.PointCloud, o3d.geometry.LineSet]
    ],
    scalebar: Union[float, int, bool] = False,
    window_name: str = "Interactive Visualization - Editable Mode Disabled",
) -> None:
    """
    ...
    if isinstance(scalebar, (float, int)) and scalebar > 0:
        ...
        points = np.concatenate(
            [
                np.asarray(geometries[i].points)
                if isinstance(
                    geometries[i], (o3d.geometry.LineSet, o3d.geometry.PointCloud)
                )
                else np.asarray(geometries[i].vertices)
        ...
        shift = (
            aabb.get_center()
            - aabb.get_extent() / 2
            - [scalebar / 2, scalebar / 2, scalebar / 2]
        )  # Shift the coordinate frame away from the bounding box
